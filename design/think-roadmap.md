...
- [sessions.md](./sessions.md) — Session integration design (the foundation)
...
**Sessions** owns all conversation data: messages, compaction overlays, search indexes, configuration. Prompt context blocks moved to `agents/context`. See [sessions.md](./sessions.md) for the full design.
...
Features that are implemented or designed and ready to implement (via existing Think code + Session integration from [sessions.md](./sessions.md)):
...
| ---------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Context blocks**                                   | `configureContext()` blocks with auto-wired SQLite providers. Model can read/write its own persistent memory via `set_context` tool. Supports readonly blocks, writable blocks, R2-backed skill collections (`load_context`), and FTS5 searchable blocks (`search_context`). | Not available. System prompt is static per-request. No LLM-writable persistent memory.                                                                                                                                                 |
...
**Already designed in:** [sessions.md](./sessions.md) — API surface is defined. Implementation is straightforward.
...
**Changes (from [sessions.md](./sessions.md)):**
